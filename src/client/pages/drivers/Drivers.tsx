...
const Drivers: React.FC = () => {
  ...
  const handleUpdateDriver = async (payload: any) => {
    try {
  ...
  return (
    <div className="flex h-screen overflow-hidden relative">
      <div className="w-2/3 p-6 overflow-y-auto border-r border-gray-300">
        ...
        {status === "failed" && (
          <div className="p-4 text-center text-red-600">Error: {String(error)}</div>
        )}
        ...
        <table className="w-full table-auto border-collapse">
          ...
          <tbody>
            {status !== "loading" && filteredDrivers.map((driver) => (
              <tr
                key={driver.id}
              ...
              >
                <td className="p-2 flex items-center gap-2">
                  <img
                    src={driver.profile_image || driver.profileImage || "https://via.placeholder.com/40"}
                    alt={driver.name}
                      ...
                      onClick={(e) => {
                        ...
                        toast((t) => (
                          <div className="flex items-center gap-3">
                            <span>Delete <b>{driver.name}</b>?</span>
                            <button
            ...
            {status !== "loading" && filteredDrivers.length === 0 && (
              <tr>
                <td colSpan={5} className="p-4 text-center">No drivers found</td>
              </tr>
            )}
          </tbody>
        </table>
        ...
        <div className="w-1/3 p-6 bg-gray-50 overflow-y-auto border-l border-gray-200">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-4">
              <img
                src={selectedDriver.profile_image || "https://via.placeholder.com/100"}
          ...
          <div className="space-y-3 mt-4">
            ...
            <p><strong>Address:</strong> {selectedDriver.address || "N/A"}</p>
          </div>

          <div className="flex gap-2 mt-6">
            <button className="bg-blue-500 text-white px-3 py-2 rounded hover:bg-blue-600 w-full">Message</button>
            <button
      ...
      {showAddForm && (
        <div className="absolute inset-0 bg-black/40 flex justify-center items-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-[400px] relative">
            <button onClick={() => setShowAddForm(false)} className="absolute top-2 right-3 text-gray-500 hover:text-gray-800 text-xl">✖</button>
            <AddDriverForm onClose={() => setShowAddForm(false)} onSubmit={handleAddDriver} />
          </div>
        </div>
      )}
      ...
      {showEditForm && selectedDriverForEdit && (
        <div className="absolute inset-0 bg-black/40 flex justify-center items-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-[400px] relative">
            <button onClick={() => setShowEditForm(false)} className="absolute top-2 right-3 text-gray-500 hover:text-gray-800 text-xl">✖</button>
            <EditDriverForm
              driver={selectedDriverForEdit}
              onClose={() => setShowEditForm(false)}
              onSubmit={(data) => handleUpdateDriver({ ...data, id: selectedDriverForEdit.id })}
            />
          </div>
        </div>
      )}
